...
function statIterator (drive, path, opts) {
  ...
  function open (cb) {
    drive.ready(err => {
      ...
      stack.unshift({ path: '/', target: null, iterator: drive._db.iterator(path, opts) })
      return cb(null)
    })
  }
    ...
    if (opts && opts.recursive || (nodePath === path)) {
      return drive.stat(linkPath, (err, targetStat, _, resolvedLink) => {
        ...
        if (targetStat.isDirectory()) {
          stack.unshift({ path: nodePath, target: resolvedLink, iterator: drive._db.iterator(resolvedLink, { gt: true, ...opts }) })
        }
        return cb(null, stat)
      })
    }
  ...
  return nanoiterator({
    open (cb) {
      drive.ready(function (err) {
        if (err) return cb(err)
        ite = drive._db.mountIterator(opts)
        return cb(null)
      })
    },
    next (cb) {
      if (first) {
        ...
        if (drive.metadata.has(0)) {
          return drive._getContent(drive._db, err => {
            if (err) return cb(err)
            return cb(null, {
              path: '/',
              metadata: drive.metadata,
              content: drive._contentStates.get(drive._db).feed
            })
          })
        } else {
          return cb(null, {
...
function readdirIterator (drive, name, opts) {
  ...
  function next (cb) {
    ite.next((err, value) => {
      ...
      const { path: statPath, stat, mount, innerPath } = value
      const relativePath = (name === statPath) ? statPath : p.relative(name, statPath)
